...
def plot_dual_timeline_data(experiments_set, latency_ax, gpu_free_ax, gpu_variance_ax, instance_ax, font_size=10,
                            provisioning_threshold=60, sigma=10):
    """
    Plot two sets of data on the same timeline with different y-axes.
    """
    initial_text_height = 90
    latency_ax.text(5000, initial_text_height, "Num over Cond", fontsize=font_size, verticalalignment='bottom')
    latency_ax.text(5500, initial_text_height + 10, "And", fontsize=font_size, verticalalignment='bottom')
    latency_ax.text(5250, initial_text_height + 20, "P99 Latency(s)", fontsize=font_size, verticalalignment='bottom')
    text_info = {}
    for exp in experiments_set:
        ...
        gpu_variance_ax.set_xlabel("Request Index", fontsize=font_size)

        latency_ax.plot([0, len(experiments_set[0]['request_latencies'])], [provisioning_threshold, provisioning_threshold],
                 color='red', linewidth=1)
        latency_ax.text(0, provisioning_threshold + 10, "Scaling Cond", color='red', fontsize=font_size,
                        verticalalignment='bottom')
        latency_ax.legend(loc='upper left', fontsize=font_size, ncol=3, bbox_to_anchor=(0, 1.25))
        gpu_free_ax.legend(loc='upper left', fontsize=font_size, ncol=3, bbox_to_anchor=(0, 1.25))
